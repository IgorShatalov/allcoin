...
public:
    ...
    bool WriteZALCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZALCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZALCSeed_deprecated(uint256& seed);
    bool EraseZALCSeed();
    bool EraseZALCSeed_deprecated();

    bool WriteZALCCount(const uint32_t& nCount);
    bool ReadZALCCount(uint32_t& nCount);
