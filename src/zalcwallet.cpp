...
CzALCWallet::CzALCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zalc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZALCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZALCSeed_deprecated()) {
                    LogPrintf("%s: Updated zALC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zalc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zalc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzALCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZALCCount(nCountLastUsed);
    else if (!walletdb.ReadZALCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzALCWallet::Lock()
{
    seedMaster = 0;
}

void CzALCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzALCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZALC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzALCWallet::LoadMintPoolFromDB()
{
...
void CzALCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzALCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzALCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zalcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzALCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZALC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zalcTracker which also adds to database
    pwalletMain->zalcTracker->Add(dMint, true);
...
void CzALCWallet::SeedToZALC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzALCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzALCWallet::GenerateDeterministicZALC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzALCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzALCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
