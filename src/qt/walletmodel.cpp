...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered allcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ALC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal allcoin:URI (allcoin:XyZ...?message=example)
            {
...
static void NotifyzALCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzALCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzALCReset.connect(boost::bind(NotifyzALCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzALCReset.disconnect(boost::bind(NotifyzALCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zalcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
