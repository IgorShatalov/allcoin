...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ALC);
    ...
    if (!settings.contains("nAnonymizeAllCoinAmount"))
        settings.setValue("nAnonymizeAllCoinAmount", 1000);

    nAnonymizeAllCoinAmount = settings.value("nAnonymizeAllCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAllCoinAmount"))
        SoftSetArg("-anonymizeallcoinamount", settings.value("nAnonymizeAllCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in allcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAllCoinAmount:
            return QVariant(nAnonymizeAllCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAllCoinAmount:
            nAnonymizeAllCoinAmount = value.toInt();
            settings.setValue("nAnonymizeAllCoinAmount", nAnonymizeAllCoinAmount);
            emit anonymizeAllCoinAmountChanged(nAnonymizeAllCoinAmount);
            break;
        case CoinControlFeatures:
